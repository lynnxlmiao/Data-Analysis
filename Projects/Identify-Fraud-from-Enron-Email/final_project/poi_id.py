...
number_of_data = len(data_dict)
print("Total number of data points: %i" % number_of_data)

# Total number of poi and non-poi
poi = 0
...
print("There are %i features for each person in the dataset, and %i features are used" \
      %(len(all_features), len(features_list)))

# Check if there is features with many missing values? etc.
missing_values = {}
