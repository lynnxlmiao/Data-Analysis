...
This is only the donors' geographical distribution. We can also see the donations amount by geographical locations and also donations per capita by geographical locations.

```{r }
#Dataframe of geographical location and total amount
df_zip_total <- FL %>%
  group_by(contbr_zip) %>%
  summarize(value = sum(contb_receipt_amt)) %>%
  arrange(desc(value))

df_zip_total$region <- df_zip_total$contbr_zip
df_zip_total <- na.omit(df_zip_total)

#Dataframe of geographical location and average amount
df_zip_avg <- FL %>%
  group_by(contbr_zip) %>%
  summarize(value = sum(contb_receipt_amt)/n())

df_zip_avg$region <- df_zip_avg$contbr_zip
df_zip_avg <- na.omit(df_zip_avg)
```

```{r}
#plot of total amount
df_zip_total_map <- ZipChoropleth$new(df_zip_total)
df_zip_total_map$ggplot_scale = scale_fill_brewer(palette = 1, drop = FALSE)
df_zip_total_map$set_zoom_zip(state_zoom = "florida",
                              county_zoom = NULL,
                              msa_zoom = NULL,
                              zip_zoom = NULL)
df_zip_total_map$title = "Florida Total Donation (thousands) by County"
df_zip_total_map$render()
```

```{r}
df_zip_avg_map <- ZipChoropleth$new(df_zip_avg)
df_zip_avg_map$ggplot_scale = scale_fill_brewer(palette = 1, drop = FALSE)
df_zip_avg_map$set_zoom_zip(state_zoom = "florida",
                            county_zoom = NULL,
                            msa_zoom = NULL,
                            zip_zoom = NULL)
df_zip_avg_map$title = "Florida Average Donation (thousands) by County"
df_zip_avg_map$render()
```
The total donation amount still can see similar trend as the donors number, but if looking at the donations per capita by county, some show higher donations per capita, for example Collier.
